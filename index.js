...
client.on("ready", () => {
    ...
    for (let i = 0; i < config.servers.length; i++) {
        if (config.servers[i].reminderTimes.includes(currentFormattedTime) && !executedTimes.has(currentFormattedTime)) {
                // Send message in updates channel
                client.channels.cache.get(config.servers[i].updateChannel).send(config.servers[i].messages[Math.floor(Math.random() * config.servers[i].messages.length)]);
